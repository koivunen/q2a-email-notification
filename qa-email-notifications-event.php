...
class qa_email_notifications_event {
	...
	function process_event($event, $userid, $handle, $cookieid, $params) {
		...
		$userids    = qa_db_select_with_pending($this->qa_db_notificaton_emails_selectspec(qa_get_logged_in_userid(), $tags, $categoryid));
		
		$emails = $this->combine_emails($userids);
		...
		for ($i = 0; $i < count($emails); $i++) {
			...
			$this->category_email_notification_send_notification($bcclist, null, null, qa_lang('emails/q_posted_subject'), qa_lang('notify/q_posted_body'), array(
	...
	function qa_db_notificaton_emails_selectspec($userid, $tags, $categoryid) {
		if ($this->plugin_viable()) { //proceed only if the plugin is enabled
			...
			if (!!qa_opt('ami_email_notf_allow_user_follower')) {
				$source .= (!!$source) ? ' UNION ' : '';
				$source .= "( SELECT ^userfavorites.userid , 'U' as favorited , ^userpoints.points from ^userfavorites JOIN ^userpoints ON ^userfavorites.userid=^userpoints.userid WHERE ^userfavorites.entityid=$ AND ^userfavorites.entitytype=$  AND ^userfavorites.userid !=$ )";
				$args      = array(
					$userid,
					QA_ENTITY_USER,
			...
			if (!!qa_opt('ami_email_notf_allow_tag_follower') && !!$tags) {
				$source .= (!!$source) ? ' UNION ' : '';
				$source .= "( SELECT ^userfavorites.userid , 'T' as favorited , ^userpoints.points from ^userfavorites JOIN ^userpoints ON ^userfavorites.userid=^userpoints.userid WHERE ^userfavorites.entityid IN 
                            ( SELECT wordid from ^words where ^words.word IN ($) ) AND ^userfavorites.entitytype=$ AND ^userfavorites.userid !=$ )";
				$args      = array(
					qa_tagstring_to_tags($tags),
					QA_ENTITY_TAG,
					qa_get_logged_in_userid()
				);
				$arguments = array_merge($arguments, $args);
			...
			if (!!qa_opt('ami_email_notf_allow_cat_follower') && !!$categoryid) {
				$source .= (!!$source) ? ' UNION ' : '';
				$source .= "( SELECT ^userfavorites.userid , 'C' as favorited , ^userpoints.points from ^userfavorites JOIN ^userpoints ON ^userfavorites.userid=^userpoints.userid " . "WHERE ^userfavorites.entityid=$ AND ^userfavorites.entitytype=$ AND ^userfavorites.userid !=$ )";
				$args      = array(
					$categoryid,
					QA_ENTITY_CATEGORY,
					qa_get_logged_in_userid()
	...
	function combine_emails($emails_id_list) {
		...
		foreach ($emails_id_list as $email_data) {
			...
			if (!$email && array_key_exists('userid', $email_data) && $email_data['userid']) {
				$email               = qa_get_user_email($email_data['userid']);
				$email_data['email'] = $email;
			}
			
			if ($email && !in_array($email, $unique_email_ids)) {
	...
	public function plugin_viable() {
		return (((!!qa_opt('ami_email_notf_allow_cat_follower')) || (!!qa_opt('ami_email_notf_allow_tag_follower')) || (!!qa_opt('ami_email_notf_allow_user_follower'))));
	}
